...
output(Basename, Messages, Options) ->
    ...
    io:format("generate header file to ~p~n",[HeaderFile]),
        ...
        fun({FNum,Tag,SType,SName,_,Default}, Acc) ->
            {cons,L,
                {call,L,{atom,L,pack},[
                    {integer,L,FNum},
                    {atom,L,Tag},
                    {call,L,{atom,L,with_default},[{record_field,L,{var,L,'Record'},
                          atomize(MsgName),{atom,L,atomize(SName)}},erl_parse:abstract(Default)]},
...
filter_decode_clause(Msgs, {MsgName, Fields}, {clause,L,_Args,Guards,[_,B,C]}) ->
    Types = lists:keysort(1, [{FNum, list_to_atom(SName), list_to_atom(SType), 
                               decode_opts(Msgs, Tag, SType)} || {FNum,Tag,SType,SName,_,_} <- Fields]),
    Cons = 
      lists:foldl(
...
collect_full_messages([{message, Name, Fields} | Tail], Acc) ->
    ...
    SubMessages = 
      lists:foldl(fun({message, C, D}, TmpAcc) -> [{message, [C | ListName], D} | TmpAcc];
                     (_, TmpAcc) -> TmpAcc
                  end, [], Fields),
...
resolve_types([{TypePath, Fields} | Tail], AllPaths, Acc) ->
    ...
    resolve_types(Tail, AllPaths,[{type_path_to_type(TypePath), lists:reverse(FieldsOut) } | Acc]);
resolve_types([], _, Acc) ->
    Acc.

write_header_include_file(HeaderFile, Messages) ->
    {ok, FileRef} = file:open(HeaderFile, [write]),
    [begin
        OutFields = [string:to_lower(A) || {_, _, _, A, _, _} <- lists:keysort(1, Fields)],
