...
start() ->
	case init:get_argument(name) of
		{ok,[[Node]]}->	
			start(list_to_atom(Node));
		_ ->	
			{error,no_node_specify}
	end.
start(Node)->
...
handle_call({sort,Sort},_From,State)->
	case get_sort(Sort) of
		undefined ->
			{reply,{error,bad_field},State};
		Tag	->	
			{reply,{ok,Sort},State#state{sort = Tag}}
	end;
...
%% 	rpc:call(State#state.node,observer_backend,etop_collect,[Self]),
	receive
		{_,#etop_info{} = Etop_info}->	
			ok;
		Other->
			io:format("updateing other ----->~p~n", [Other]),
			Etop_info = error,
			exit({observer_backend,timeout})
	after
		1000->
			Etop_info = error,
			exit({observer_backend,timeout})
...
write_proc_head()->
	io:format("PidName or Initial Func    		Time    Reds  Memory    MsgQ Current Function~n",[]).
writepinfo([#etop_proc_info{
								pid=Pid,
								mem=Mem,
								reds=Reds,
								name=Name,
