...
req_handle(Req) ->
  ...
  %%获取要重启的节点
  NodeIndex = dd_util:to_list(proplists:get_value("node", GetData, undefined)),
  _Node = adminserver_cache_proxy:get_restart_node(NodeIndex),
  %%调用外部restart重启
%%   Params = 
  Result = my_exec("base/restart "),
  case Result of
	  success->
		  {struct, [{<<"result">>, 0}]};
	  _->
