...
start(normal, _StartArgs) ->
    ...
    wait_mnesia_table(),
  	{ok, _} = dd_config:start_link(os:getenv("ADMIN_CFG_PATH")),
    {ok, Pid} = adminserver_sup:start_link(),
    log_running_applications(),
    ?FILE_LOG_INFO("adminserver ~s is started in the node ~p", [?MYTEST_VERSION, node()]),
    {ok, Pid};
...
stop(_State) ->
    ...
    ?FILE_LOG_INFO("adminserver ~s is stopped in the node ~p", [?MYTEST_VERSION, node()]),
    ok.
