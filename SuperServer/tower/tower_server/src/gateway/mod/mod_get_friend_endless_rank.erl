...
%%%-------------------------------------------------------------------
-module(mod_get_friend_endless_rank).
-author("zqlt").
...
req_handle(Req) ->
  ...
  {success, Uin} = gateway_util:get_uin_by_session(SessionID, get_friend_endless_rank),

  {success, CacheNode} = gateway_util:get_cache_node(Uin),

  case rpc:call(CacheNode, cache, get_friend_endless_rank, [Uin]) of
    {success, DataBin} ->
      {
        struct,
        [
