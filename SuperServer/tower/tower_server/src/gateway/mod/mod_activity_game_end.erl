...
req_handle(Req) ->
  ...
  ?FILE_LOG_DEBUG("activity tollgate game_end => session = ~p, ~p", [SessionId, GameData]),
  {struct, DataList} = mochijson2:decode(GameData),
  {success, GameEndData} = gateway_util:decode_game_end_data(DataList),
  gateway_util:check_activity_tollgate(GameEndData#game_end.tollgate_id),
