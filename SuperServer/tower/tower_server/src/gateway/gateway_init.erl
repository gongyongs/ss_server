...
mnesia_init(Node) when is_atom(Node) ->
  ...
  case {lists:member(gateway, KnownTabs)} of
    {true} ->
      io:format("~n ==> copy gateway. ~n", []),
      case mnesia:add_table_copy(gateway, node(), ram_copies) of
        {atomic, ok} ->
          io:format("~n ==> copy gateway success. ~n", []);
        {aborted, Reason1} ->
          io:format("~n ==>  copy gateway fail reason=~p. ~n", [Reason1])
      end;
    _ ->
      io:format("==> Error# can't find all tables in the master node!~n", [])
  end,
