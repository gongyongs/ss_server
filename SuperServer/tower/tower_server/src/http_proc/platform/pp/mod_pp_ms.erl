...
complete_orders(OrderID, BillNo, AccountID, AmountPPCoin, ProductId, OrderFlag,  Rationality) ->
  gen_server:call(?MODULE, {complete_orders, {AccountID, {OrderID, BillNo, AccountID, AmountPPCoin, ProductId, OrderFlag, Rationality}}}).
...
handle_call(read_public_key, _From, State) ->
  Ret =
    case dd_config:get_cfg(pp_public_key) of
      {success, Value} -> {success, Value};
      fail ->
        Path = dd_util:to_list(os:getenv("KEY_DIR")) ++ "pp_key.key",
        case read_pb_key(Path) of
          {success, Key} ->
            dd_config:write_cfg(pp_public_key, Key),
            {success, Key};
          fail -> fail
        end
    end,
...
order_work_func() ->
  TableID = ets:new(order_pp, [set, protected, {keypos, #rd_order_pp.bill_no}]),
  ...
  try
    receive
      {execute, From, {FuncName, Param}} ->
        Ret = mod_pp_ms_impl:execute(TableID, FuncName, Param),
    ...
    after
      30*1000 ->
        ...
        MatchSpec = [{#rd_order_pp{time_stamp = '$1', _='_'}, [{'=<', '$1', CurTime}], [true]}],
