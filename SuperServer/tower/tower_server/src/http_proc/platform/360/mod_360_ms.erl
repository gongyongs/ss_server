...
complete_orders(OrderID, BillNo, AccountID, Amount, ProductId, Flag,  Rationality) ->
  gen_server:call(?MODULE, {complete_orders, {AccountID, {OrderID, BillNo, AccountID, Amount, ProductId, Flag, Rationality}}}).
%%%===================================================================
%%% API
%%%===================================================================
...
order_work_func() ->
  TableID = ets:new(order_360, [set, protected, {keypos, #rd_order_360.bill_no}]),
  ...
  try
    receive
      {execute, From, {FuncName, Param}} ->
        Ret = mod_360_ms_impl:execute(TableID, FuncName, Param),
    ...
    after
      30*1000 ->
        ...
        MatchSpec = [{#rd_order_360{time_stamp = '$1', _='_'}, [{'=<', '$1', CurTime}], [true]}],
