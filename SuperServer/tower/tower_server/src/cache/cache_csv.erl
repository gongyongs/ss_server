...
%%SuperStar
-export([
	get_player_piece_config/1,
	get_item_base_info/1,
	get_rm_buff_info/1,
	get_rm_battle_info/1,
	get_rm_battle_random_id/1,
	get_rm_buff_random_id/1
]).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%SuperStar项目配置文件不采用从ets中读取的机制，采用从代码文件中读取。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
get_player_piece_config(Id)->
	case playerPieceInfo:get_data(Id) of
		[]->
			?FILE_LOG_WARNING("get_player_piece_config fail", []),
      		throw({custom, <<"HintSystemDataError">>});
		PlayerPieceConfig->
			PlayerPieceConfig
	end.

get_item_base_info(Id)->
	ID_int = dd_util:to_integer(Id),
	case itemBaseInfo:get_data(ID_int) of
		[]->
			?FILE_LOG_WARNING("get_item_base_info fail ~p", [Id]),
      		throw({custom, <<"HintSystemDataError">>});
		Info->
			Info
	end.
...
get_rm_buff_info(Id)->
	case rm_buff_data:get_data(Id) of
		[]->
			?FILE_LOG_WARNING("get_rm_buff_info fail ~p", [Id]),
      		throw({custom, <<"HintSystemDataError">>});
		Info->
			Info
	end.

get_rm_buff_list_debuff(IsDebuff)->
	AllIDList = rm_buff_data:get_indexs(),
	InfoList = 
	lists:filter(
	  fun(ID) -> 
			BattleInfo = rm_buff_data:get_data(ID),
	  		if
			  BattleInfo#rm_buff_data.isdebuff =:= IsDebuff->true;  
			  true -> false
			end
	  end, AllIDList).

get_rm_buff_random_id(IsDebuff) ->
	TypeList = get_rm_buff_list_debuff(IsDebuff),
	Length= length(TypeList),
	Index = dd_util:random_in_range(1,Length),
	lists:nth(Index,TypeList).
	

get_rm_battle_info(Id)->
	case rm_battle_data:get_data(Id) of
		[]->
			?FILE_LOG_WARNING("get_rm_battle_info fail ~p", [Id]),
      		throw({custom, <<"HintSystemDataError">>});
		Info->
			Info
	end.

get_rm_battle_type_list(TYPE)->
	AllBattleID = rm_battle_data:get_indexs(),
	InfoList = 
	lists:filter(
	  fun(ID) -> 
			BattleInfo = rm_battle_data:get_data(ID),
	  		if
			  BattleInfo#rm_battle_data.type =:= TYPE->true;  
			  true -> false
			end
	  end, AllBattleID).

get_rm_battle_random_id(TYPE) ->
	TypeList = get_rm_battle_type_list(TYPE),
	Length= length(TypeList),
	Index = dd_util:random_in_range(1,Length),
	lists:nth(Index,TypeList).
