...
-define(DEFAULT_DATABASE, log_stat).
...
%%% API
%%%===================================================================
execute(FunName,FuncParam) ->
  gen_server:call(?MODULE, {execute, {FunName,FuncParam}}).
...
handle_call({execute, {Name, FuncParam}}, From, #state{work_proc_pool = WorkPool}=State) ->
  {Work, NewWorkPool} = get_work_proc(WorkPool),
  ?FILE_LOG_DEBUG(" => Work is ~p, NewWorkPool is ~p",[Work, NewWorkPool]),
  Work ! {execute, {From, {Name, FuncParam}}},
...
connect_db(PoolCount) ->
  {success, DbCfgList} = dd_config:get_cfg(db_list3),
  %%开启进程，连接数据库
  spawn(
    fun() ->
      lists:map(
...
work_proc(DbName) ->
  receive
    ...
    {execute, {From, {FuncName, FuncParam}}} when is_atom(FuncName) ->
      RetValue =
        try
          log_db_stat_proc:execute(DbName, {FuncName,FuncParam})
        catch
