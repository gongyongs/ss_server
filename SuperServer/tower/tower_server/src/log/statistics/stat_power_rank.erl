...
%%%
%%% @end
%%% Created : 30. 十月 2014 下午8:13
%%%-------------------------------------------------------------------
-module(stat_power_rank).
-author("zqlt").
...
read_data_to_list(Index, Len, OutList) ->
  ...
  case rpc:call(Node, cache, stat_power_rank, [Index, Len]) of
    {success, DataList} ->
      PayDataList =
        lists:map(
          fun({Uin, DisName, Atk, PlayDays, PayVal}) ->
            #power_rank_item{uin = Uin, dis_name = DisName, atk = Atk, play_days = PlayDays, pay_val = PayVal}
          end, DataList),
      SortFunc =
        fun(#power_rank_item{atk = LV}, #power_rank_item{atk = RV}) ->
          if
            LV > RV -> true;
            true -> false
          end
...
merge_1([#power_rank_item{atk = LV} = L | LT], [#power_rank_item{atk = RV} = R | RT], OutList) when LV > RV -> merge_1(LT, [R |RT], [L | OutList]);
merge_1([L | LT], [R | RT], OutList) -> merge_1([L | LT], RT, [R | OutList]).

output_Log(List, Interval) when is_list(List) ->
  FileName = log_statistics_util:get_output_filename("power_rank", Interval),
  {ok, F} = file:open(FileName, write),
  io:format(F, "~s\t~s\t~s\t~s\t~s~n", ["UserID", "UserName", "UserAtk", "UserPlayDays", "UserTotalPayAmount"]),
  lists:foreach(
    fun(Item) ->
      io:format(F, "~p\t~s\t~p\t~p\t~p~n", [Item#power_rank_item.uin, Item#power_rank_item.dis_name, Item#power_rank_item.atk, Item#power_rank_item.play_days, Item#power_rank_item.pay_val])
    end, List),
  file:close(F).
