...
start(normal, _StartArgs) ->
  ...
  {ok, _} = dd_config:start_link(os:getenv("RANK_CFG_PATH")),

    {ok, Pid} = ranking_sup:start_link(),
    log_running_applications(),
    ?FILE_LOG_INFO("ranking ~s is started in the node ~p", [?RANKING_VERSION, node()]),
    {ok, Pid};
...
stop(_State) ->
    ...
    ?FILE_LOG_INFO("ranking ~s is stopped in the node ~p", [?RANKING_VERSION, node()]),
    ok.
