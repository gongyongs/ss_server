...
-define(VERIFY_URL, "http://passport_i.25pp.com:8080/account?tunnel-command=2852126760").
-define(GAME_ID, 4931).
-define(APP_KEY, "7e228763ade8aba96e28c393a076c525").
...
login_verify(Sid) when is_list(Sid) andalso length(Sid) > 0->
  ...
  MD5 = dd_util:md5_string("sid=" ++ Sid ++ ?APP_KEY),
  ReqData =
    {
      struct,
      [
        ...
        {<<"game">>,{struct,[{<<"gameId">>, ?GAME_ID}]}},
        {<<"encrypt">>, <<"md5">>},
        {<<"sign">>, dd_util:to_binary(MD5)}
      ]
    },
  Value = mochijson2:encode(ReqData),
 dispatch_util:http_post_req(?VERIFY_URL, "application/json;charset=utf-8", Value,fun mod_pp_login:login_verify_call_back/2, ID).
