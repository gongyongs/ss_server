...
start(normal, _StartArgs) ->
  ...
  wait_for_tables(),
    ...
    {ok, _} = dd_config:start_link(os:getenv("DATABASE_CFG_PATH")),
    {ok, Pid} = database_sup:start_link(),
    log_running_applications(),
    ?FILE_LOG_INFO("database ~s is started in the node ~p", [?DATABASE_VERSION, node()]),
    {ok, Pid};
...
stop(_State) ->
    ...
    ?FILE_LOG_INFO("database ~s is stopped in the node ~p", [?DATABASE_VERSION, node()]),
    ok.
