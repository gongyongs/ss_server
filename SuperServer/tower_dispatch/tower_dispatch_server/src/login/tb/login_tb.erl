...
execute(login_by_uname, {UName, "TB", DisName, Device}) ->
  ?FILE_LOG_DEBUG("login_by_uname => uname = ~p, plat = ~p, dis_name = ~p, device = ~p", [UName, "TB", DisName, Device]),
  uname = get(mode),
  key_check(UName),
    ...
    [] ->
      case rpc:call(DBNode, database_monitor, execute, [login_by_uname, UName]) of
        {success, UserInfo} ->
          ...
          player_id = UName, plat_type = "TB"}, add_ts = dd_util:timestamp()},
