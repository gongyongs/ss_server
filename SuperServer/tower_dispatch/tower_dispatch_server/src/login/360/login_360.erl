...
execute(login_by_uname, {UName, "360", DisName, Device}) ->
  ?FILE_LOG_DEBUG("login_by_uname => uname = ~p, plat = ~p, dis_name = ~p, device = ~p", [UName, "360", DisName, Device]),
  uname = get(mode),
  key_check(UName),
      ...
      case rpc:call(DBNode, database_monitor, execute, [login_by_uname, UName]) of
        {success, UserInfo} ->
          NUserInfo = UserInfo#user_info{uname = UName, dis_name = DisName, device = Device, addition = #addition{}, platform_info = #platform{player_dis_name = DisName, plat_friend = [],
            player_id = UName, plat_type = "360"}, add_ts = dd_util:timestamp()},
